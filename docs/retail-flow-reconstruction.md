...
All 100 encounter records duplicate their signed 32-bit position, and 67 are active (`renderModelId >= 0`). The leading part of the 2,048-byte table begins with 21 start/count pairs—exactly the number of `2003` course chunks—and partitions the encounter records once, in order, from 0 through 99. This establishes the authored encounter ordering and course-chunk grouping without yet assigning human-readable pickup or hazard names. The browser currently renders the 170 active world entities using original transforms. Encounter identifiers `30–85` are consumed through the retail sprite/animation renderer and must not be treated as indices into the 80-object `2004` TMD; the correct mapping to the original `2005` TIM frames is still being decoded. Event-state activation, sprite animation, motion, and collision responses remain the next implementation step.
