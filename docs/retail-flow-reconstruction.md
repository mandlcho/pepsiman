...
The two fourteen-record destination tables at executable addresses `0x8007ae1c` and `0x8007aed0` are retained in `assets/ripped/retail-flow.json`, generated by `tools/extract_retail_flow.py`. A later byte write of value `27` targets a different player-structure field and is not treated as proof that animation clip 27 plays. The browser clear overlay now uses the original eight-piece PIC results backdrop and the original `SCENE 1`/`CLEAR` glyphs; its numeric counter layout remains provisional. States 4–8, their camera/effect object, transition wait, and the complete retail results behavior remain under reconstruction.

Event record 194 drives a separate five-state Stage 1 set-piece through overlay controller `0x800f7584`. It disables player input, preserves the initial player/camera transforms, and linearly centers X/Z over 25 frames at executable-table game position `(-5700, -9080, 18500)`, corresponding to browser position `(-5700, 9080, -18500)`. After a one-frame transition it runs a nine-frame sound phase, invoking retail sound ID 56 on alternating early frames. It then submits 40 effects at ten-unit vertical intervals, changes player runtime state, restores control, and runs a closing screen effect for 30 frames. The target and proven numeric sequence are exported under `stageOneScriptedEvents`; the effect graphics and semantic label remain unassigned until their runtime registrations are traced.
