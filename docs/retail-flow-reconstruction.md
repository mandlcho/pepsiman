...
- 100 encounter records of 60 bytes at `0xe798`, followed by a 2,048-byte course-chunk index/table at `0xff08`;
...
All 100 encounter records duplicate their signed 32-bit position, and 67 are active (`renderModelId >= 0`). The leading part of the 2,048-byte table begins with 21 start/count pairs—exactly the number of `2003` course chunks—and partitions the encounter records once, in order, from 0 through 99. This establishes the authored encounter ordering and course-chunk grouping without yet assigning human-readable pickup or hazard names. The browser renders the 170 active world entities using original transforms and now also renders the 59 active encounters whose retail model identifiers resolve to the 80-object `2004` TMD. Eight encounter records use identifiers `80` or `85`; their separate retail model source remains to be decoded rather than substituting incorrect geometry. Event-state activation, motion, and encounter collision responses remain the next implementation step.
